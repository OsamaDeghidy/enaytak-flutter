...
class LabHomeScreen extends StatefulWidget {
  ...
  @override
  _LabHomeScreenState createState() => _LabHomeScreenState();
}
...
class _LabHomeScreenState extends State<LabHomeScreen> {
  ...
  Future<void> fetchStaffProfile() async {
    try {
      ...
      final staffResponse = await http.get(
        Uri.parse('http://67.205.166.136/api/labs/$specificId/'),
        headers: {
  ...
  Future<void> fetchConfirmedAppointments() async {
    ...
    final url =
        'http://67.205.166.136/api/labs/$specificId/appointments/?page=1';
    try {
